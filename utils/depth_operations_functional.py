...
#         return indices
#
#     indices = patches_indices_meshgrid_no_b(pl_nchw, h, w)
#     patches = ks.layers.Lambda(lambda x: tf.map_fn(lambda y: tf.cast(tf.gather_nd(y, indices, batch_dims=0), dtype=tf.float16), x, dtype=tf.float16))(pl_nchw)
#     patches = tf.transpose(patches, perm=[1, 0,2])
#     patches = tf.reshape(patches, [patches.shape[0], -1])
